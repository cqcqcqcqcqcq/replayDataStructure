...
static int shrinkDynamicCapacity(dynamicArray *pArray)
{
    int needChrinkCapacity = pArray->capacity - (pArray->capacity >> 1);

    ELEMENTTYPE * tmpPtr = pArray->data;
    pArray->data = (ELEMENTTYPE *)malloc(sizeof(ELEMENTTYPE) * needChrinkCapacity);
    if (pArray->data == NULL)
    {
        return MALLOC_ERROR;
    }
    ...
    /*更新容量*/
    pArray->capacity = needChrinkCapacity;
    return ON_SUCCESS;
